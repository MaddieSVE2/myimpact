...
- impact_records kind/location_json/reporting_year (migration 0043) also existed as a file but was never applied to dev DB — e2e suites run against dev DATABASE_URL, so "column does not exist" in e2e logs means run the migration file via psql and insert its name into _migrations.
