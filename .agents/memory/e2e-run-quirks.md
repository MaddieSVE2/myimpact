...
- NEVER set `E2E_TEST_MODE` in the persistent dev environment (`.replit` userenv / setEnvVars development scope). It silently disables real magic-link email sends on the dev API server, which surfaces to the user as "Resend stopped working". Pass it inline per validation command only. This happened once (July 2026) and was removed via deleteEnvVars.
- Dev DB schema can drift behind checked-in migrations after merges (e.g. missing `birth_month` column made every reset-user call 500 across all suites). If suites fail en masse with a "column ... does not exist" in the WebServer logs, run `pnpm --filter @workspace/db run migrate` (NOT `push` — it's interactive) and re-run.
