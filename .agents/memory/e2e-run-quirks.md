...
- Specs have drifted from the app before (stale hero text, removed routes like `/api/impact/records`); the whole suite has since been realigned and passes as the `e2e` validation. If a spec fails, cross-check its expected text/route against the current tree before assuming an app regression. Judge e2e runs by whether *newly touched* areas' specs pass.
- **Do not register multiple Playwright validations at once** without unique ports: `mark_task_complete` runs ALL registered validation commands concurrently, and they all bind the same 5000/3000 ports by default — specs that pass in isolation then flake with waitForURL timeouts. Keep at most one e2e validation registered, or give each unique ports, or run the targeted suite manually via `startValidationRun({commandIds: [...]})` and skip the completion validation with a reason.
