...
- **Do not register multiple Playwright validations at once**: `mark_task_complete` runs ALL registered validation commands concurrently, and they all bind the same 5000/3000 ports — specs that pass in isolation then flake with waitForURL timeouts. Keep at most one e2e validation registered, or run smoke checks via one-off validation runs and clear them after.
