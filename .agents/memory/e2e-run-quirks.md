...
- The platform validation runner launches all four Playwright suites (e2e, smoke, subnav-alignment, e2e-edit-record) in parallel; under this contention navigation times out at the login page even though every suite passes when run serially via its workflow. Verify serially and skip validation with an explanation if the parallel run keeps flaking.
