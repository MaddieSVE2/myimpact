...
*   **WCAG AA gate:** Run `pnpm a11y` whenever publishing new or changed public content and pages. It audits every route in `PRERENDER_PAGES` with Lighthouse's automated accessibility checks and requires a 1.0 score. Keep `PUBLIC_ROUTES` in `lighthouserc.accessibility.cjs` aligned with `PRERENDER_PAGES`. Automated checks supplement, not replace, keyboard, focus, zoom, screen-reader, alt-text, heading-order, and plain-language review.
