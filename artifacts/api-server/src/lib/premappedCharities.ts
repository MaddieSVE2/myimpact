...
export async function generateForAuthority(localAuthority: string, country: string): Promise<void> {
  ...
  await db
    ...
    .set({ status, lastGeneratedAt: new Date(), updatedAt: new Date() })
 ...
 * refresh window. Runs authorities sequentially — this is a slow background
 * sweep, not a latency-sensitive path.
 */
export async function runPremappedRefreshSweep(): Promise<number> {
  ...
  const stale = await db
    .select()
    .from(localCharityAreasTable)
    .where(lt(localCharityAreasTable.lastGeneratedAt, cutoff));
