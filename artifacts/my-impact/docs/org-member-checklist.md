...
1. **Quick Log an activity**: create a fast personal record, then use **Review & share** when offered. Consented-logging organisations share automatically according to the member's consent.
...
4. **Build or update my impact record**: use the guided wizard for a fuller personal record with hours and estimated social value.
...
|---|---|---|---|
| Quick Log an activity | Quick Log, then "Review & share" when offered → `/org/share-report/:recordId` | "Your organisation" panel card 1 → `/quick-log` | Card 1 → `/quick-log` |
...
| Build or update my impact record | Account dropdown ("Calculate my impact") + orange "Calculate" CTA | Panel card 4 → `/wizard/actions` | Card 4 → `/wizard/actions` |
...
- Recording and sharing are separate ideas. Quick Log creates a fast personal record. The wizard builds or updates a fuller personal record. Explicit sharing is labelled "Review & share"; consented logging says that sharing happens automatically according to the member's consent.
- The shared `OrgMemberActionCards` component is the canonical source for both member blocks. Keep view-specific data and navigation outside it.
...
3. On `/`, confirm the "Your organisation" panel renders four cards in this order: Quick Log an activity, Open a pulse, Active challenges, Build or update my impact record.
